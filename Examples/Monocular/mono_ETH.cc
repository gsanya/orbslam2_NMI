...
int main(int argc, char **argv)
{
	...
	if (argc != 4)
	{
		cerr << endl << "Usage: ./mono_ETH path_to_vocabulary path_to_settings path_to_sequence" << endl;
		return 1;
	}
...
void LoadImages(const string &strPathToSequence, vector<string> &vstrImageFilenames, vector<double> &vTimestamps)
{
	...
	ifstream fTimes;
	string strPathTimeFile = strPathToSequence + "/_times.txt";
	...
	//there are as many pictures as many timestamps
	...
	{
		stringstream ss;
		ss << setfill('0') << setw(5) << 1 + i;		//pt1
		//ss << setfill('0') << setw(5) << 20513 + i;		//pt2
		//ss << setfill('0') << setw(5) << 52500 + i;	//pt3
		//ss << setfill('0') << setw(5) << 68700 + i;	//pt4
