...
bool loadOBJ(
	...
	while (1) {
		...
		else if (strcmp(lineHeader, "vt") == 0)//egyenlõek
		{
			glm::vec2 uv;
			fscanf(file, "%f %f\n", &uv.x, &uv.y);
			//uv.y = -uv.y; // Invert V coordinate since we will only use DDS texture, which are inverted. Remove if you want to use TGA or BMP loaders.
...
{
	printf("Loading XYZ file %s...\n", offset_path);
	std::ifstream offset_in;
	offset_in.open(offset_path);
	if (!offset_in.is_open()) {
		printf("Impossible to open the file ! Are you in the right path ? See Tutorial 1 for details\n");
		getchar();
		return false;
	}
	double offset_x, offset_y, offset_z;
	offset_in >> offset_x >> offset_y >> offset_z;
	...
	while (!in.eof()) {
		...
		in >> x >> y >> z >> color.x >> color.y >> color.z;
		vertex.x = x - offset_x;
		vertex.y = y - offset_y;
		vertex.z = z - offset_z;
		out_vertices.push_back(vertex);
		out_colors.push_back((1.0f / 256.0f)*color);
	}
	return true;
}

bool loadOBJ_PointCloud(
	const char * path,
	std::vector<glm::vec3> & out_vertices
) {
	printf("Loading OBJ file %s...\n", path);
	...
	while (1) {
		...
		if (strcmp(lineHeader, "v") == 0) {
			...
			/*max = max.x < vertex.x ? vertex : max;
			min = min.x > vertex.x ? vertex : min;*/
			out_vertices.push_back(vertex);
		}
		else {
			// Probably a comment, eat up the rest of the line
